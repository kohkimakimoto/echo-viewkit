...
func newExecutionContext(tpl *Template, ctx Context) *ExecutionContext {
	return newExecutionContextWithEchoContext(tpl, ctx, nil)
}

func newExecutionContextWithEchoContext(tpl *Template, ctx Context, eCtx echo.Context) *ExecutionContext {
	privateCtx := make(Context)

	// Make the pongo2-related funcs/vars available to the context
	privateCtx["pongo2"] = pongo2MetaContext

	// Make the template meta information available to the context
	privateCtx["template"] = map[string]any{
		"Id":   tpl.id,
		"Name": tpl.name,
	}
...
func NewChildExecutionContext(parent *ExecutionContext) *ExecutionContext {
	newctx := &ExecutionContext{
		...
		echoContext: parent.echoContext,
	}
